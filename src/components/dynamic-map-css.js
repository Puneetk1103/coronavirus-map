...
const getMaxAndMinCountryDataValues = (countryData) => {
  ...
  Object.keys(countryData).forEach((key) => {
    ...
    const value = countryData[key].deaths;
...
export const getBaseCss = ({ legendHeaderBackgroundColor, legendContentBackgroundColor, legendFontColorHeader, legendFontColorContent, legendBorderRadius, legendBorderColor, legendBoxShadow }) => (
  `.vue-world-map .land{
    fill: #B8FCB4;
    stroke:#B8FCB4;
  }
...
export const getDynamicMapCss = (countryData, chromaScale, highColor, chromaScaleOn) => {
  ...
  Object.keys(countryData).forEach((key) => {
    ...
    const value = countryData[key].cases;
